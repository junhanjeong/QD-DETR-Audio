...
"""
Training script for QD-DETR with UMT-style video-audio fusion
"""
import os
import time
import json
...
from qd_detr.start_end_dataset_umt import StartEndDataset_UMT, start_end_collate_umt, prepare_batch_inputs_umt
from qd_detr.model_umt import build_qddetr_umt
from qd_detr.inference import eval_epoch, setup_model
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        # Use UMT-specific batch preparation
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    # UMT always uses UMT-specific collate function
...
def start_training():
    ...
    # UMT dataset configuration (always uses audio features)
    dataset_config = dict(
        dset_name=opt.dset_name,
        data_path=opt.train_path,
        v_feat_dirs=opt.v_feat_dirs,
        q_feat_dir=opt.t_feat_dir,
        a_feat_dir=opt.a_feat_dir,  # Audio features required for UMT
    ...
    train_dataset = StartEndDataset_UMT(**dataset_config)
    
    # Update feature dimensions from dataset
    opt.v_feat_dim = train_dataset.v_feat_dim
    opt.a_feat_dim = train_dataset.a_feat_dim
    ...
    if opt.eval_path is not None:
        ...
        eval_dataset = StartEndDataset_UMT(**dataset_config)
    else:
        eval_dataset = None

    # Build UMT model and setup optimizer (use custom UMT model builder)
    model, criterion = build_qddetr_umt(opt)
    logger.info(f"Model {model}")
    count_parameters(model)
    logger.info("Start Training...")
    
    # Use the same optimizer/scheduler setup as original
    if opt.device.type == "cuda":
        logger.info("CUDA enabled.")
        model.to(opt.device)
